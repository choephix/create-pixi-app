...
export class MSDFText extends PIXI.Mesh {
  ...
  public get textMetric(): PIXI.Rectangle {
  ...
  public updateText() {
    ...
    for (let i = 0; i < this._text.length; i++) {
      ...
      if (!updatedchar) {
        updatedchar = MSDFCharInfo.Create(
          line,
          charCode,
  ...
  public updateUniforms() {
    ...
    this.shader.uniforms.u_color = hex2rgba(this._font.color);
    ...
    this.shader.uniforms.strokeColor = hex2rgba(this._font.strokeColor);
    this.shader.uniforms.tint = hex2rgba(this.tint);
    this.shader.uniforms.hasShadow = this._font.dropShadow;
    this.shader.uniforms.shadowOffset = this.soffsetarray;
    this.shader.uniforms.shadowColor = hex2rgba(this._font.dropShadowColor);
    ...
    this.shader.uniforms.tint = [255, 255, 255, 255];
    
    this.shader.uniforms.u_color = [255, 255, 0, 255];
    this.shader.uniforms.strokeColor = [255, 0, 0, 255];

    this.shader.uniforms.u_color = [255, 255, 255, 255];
    this.shader.uniforms.strokeColor = [0, 0, 0, 255];

    // this.shader.uniforms.shadowColor = [0, 0, 1, 0];
    window.u = this.shader.uniforms;

    setInterval(
        () => {
            // this.shader.uniforms.strokeWeight = 0.25 + Math.random() * Math.random() * 0.5;
            // this.shader.uniforms.u_weight = .25 + Math.random() * Math.random() * .5;
        }, 100
    )
  }
